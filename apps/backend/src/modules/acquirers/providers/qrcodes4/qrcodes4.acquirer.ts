...
// Quarta conta BaassPago (Versell) — mesma API, credenciais e certificado
// mTLS diferentes das outras três (qrcodes/qrcodes2/qrcodes3.acquirer.ts,
// que não são tocadas por este arquivo).
const CASH_IN_URL = 'https://api.pix.basspago.com.br';

// Carrega o certificado mTLS uma vez em memória, de uma pasta separada das
// outras contas — rejectUnauthorized:false necessário porque o servidor usa
// CA privada da ONZ. A segurança mTLS vem do certificado CLIENT apresentado.
function loadMtlsAgent(): https.Agent {
  const certsDir = path.resolve(process.cwd(), 'certs', 'basspago4');
  try {
    return new https.Agent({
      cert: fs.readFileSync(path.join(certsDir, 'VERSELL_1.crt')),
      key:  fs.readFileSync(path.join(certsDir, 'VERSELL_1.key')),
...
export class QRCodes4Acquirer implements IAcquirer {
  readonly slug = 'qrcodes4';
  ...
  async createPix(
    ...
    if (!credentials.pixKey) {
      throw new Error('Chave PIX não configurada para BaassPago (QRCodes4)');
    }
    ...
    // solicitacaoPagador é o campo exibido pro pagador no app do banco (BCB spec,
    // limite de 140 caracteres) — usado só pro nome do produto, sem vazar o
    // externalId (leadId interno) que não faz sentido pro cliente final.
    if (customer.productName) {
      body.solicitacaoPagador = customer.productName.slice(0, 140);
    }
    ...
    try {
      ...
      throw new Error(`BaassPago4 /cob falhou: ${detail}`);
    }
  ...
  async validateCredentials(credentials: AcquirerCredentials): Promise<boolean> {
    try {
      ...
      // Então registramos ${base}/qrcodes4 e eles chamam ${base}/qrcodes4/pix
      if (credentials.pixKey) {
        const base = (process.env.TELEGRAM_WEBHOOK_URL ?? 'http://localhost:3001/api/webhooks/telegram')
          .replace(/\/telegram$/, '');
        const webhookUrl = `${base}/qrcodes4/pix`;
