...
// Goldrex Finance — revenda/marca sobre a mesma infra BaassPago (Cliconbr/ONZ)
// já usada por qrcodes/qrcodes2/qrcodes3.acquirer.ts. Confirmado byte-a-byte
// contra a doc oficial (docs.goldrex.com.br): mesma URL base, mesmo fluxo
// OAuth2 client_credentials form-urlencoded e mesmo contrato /cob padrão BCB.
// Credenciais e certificado mTLS são próprios da Goldrex, independentes das
// outras contas.
const CASH_IN_URL = 'https://api.pix.basspago.com.br';

// Carrega o certificado mTLS uma vez em memória. rejectUnauthorized:false é
// necessário porque o servidor usa CA privada da ONZ Software (mesma CA das
// outras contas BaassPago) — a segurança mTLS vem do certificado CLIENT
// apresentado em cada requisição, não da validação da CA do servidor.
function loadMtlsAgent(): https.Agent {
  const certsDir = path.resolve(process.cwd(), 'certs', 'goldrex');
  try {
    return new https.Agent({
      cert: fs.readFileSync(path.join(certsDir, 'GOLDREX_1.crt')),
      key:  fs.readFileSync(path.join(certsDir, 'GOLDREX_1.key')),
...
export class GoldrexAcquirer implements IAcquirer {
  readonly slug = 'goldrex';
  ...
  private async getToken(credentials: AcquirerCredentials): Promise<string> {
    ...
    // Goldrex/BaassPago: client_id/client_secret no body form-urlencoded (sem Basic Auth)
    const body = [
  ...
  async createPix(
    ...
    if (!credentials.pixKey) {
      throw new Error('Chave PIX não configurada para Goldrex');
    }
    ...
    try {
      ...
      throw new Error(`Goldrex /cob falhou: ${detail}`);
    }

    // Busca pixCopiaECola: pode vir direto ou só via location
  ...
  async validateCredentials(credentials: AcquirerCredentials): Promise<boolean> {
    try {
      ...
      // Então registramos ${base}/goldrex e eles chamam ${base}/goldrex/pix
      if (credentials.pixKey) {
        const base = (process.env.TELEGRAM_WEBHOOK_URL ?? 'http://localhost:3001/api/webhooks/telegram')
          .replace(/\/telegram$/, '');
        const webhookUrl = `${base}/goldrex/pix`;
