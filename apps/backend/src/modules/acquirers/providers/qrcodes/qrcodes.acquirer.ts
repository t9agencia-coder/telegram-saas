...
// URL base da API PIX BaassPago (Sulcredi/Cliconbr)
const CASH_IN_URL = 'https://api.pix.basspago.com.br';

// Carrega o certificado mTLS uma vez em memória.
// rejectUnauthorized:false é necessário porque o servidor usa CA privada da ONZ.
// A segurança mTLS vem do certificado CLIENT apresentado em cada requisição.
...
export class QRCodesAcquirer implements IAcquirer {
  readonly slug = 'qrcodes';
  ...
  async createPix(
    ...
    if (!credentials.pixKey) {
      throw new Error('Chave PIX não configurada para BaassPago (QRCodes)');
    }
    ...
    try {
      ...
      throw new Error(`BaassPago /cob falhou: ${detail}`);
    }
  ...
  async validateCredentials(credentials: AcquirerCredentials): Promise<boolean> {
    try {
      ...
      // Então registramos ${base}/qrcodes e eles chamam ${base}/qrcodes/pix
      if (credentials.pixKey) {
        const base = (process.env.TELEGRAM_WEBHOOK_URL ?? 'http://localhost:3001/api/webhooks/telegram')
          .replace(/\/telegram$/, '');
        const webhookUrl = `${base}/qrcodes/pix`;
