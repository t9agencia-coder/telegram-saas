...
// Terceira conta BaassPago (Sulcredi/Cliconbr) — mesma API, credenciais e
// certificado mTLS diferentes das outras duas (qrcodes.acquirer.ts e
// qrcodes2.acquirer.ts, que não são tocadas por este arquivo).
...
// Enquanto o certificado ainda não foi colocado em certs/basspago3/, cai no
// agente sem certificado (mesmo fallback já usado em qrcodes2) — não quebra o
// deploy, só significa que essa conta ainda não está operacional de verdade.
function loadMtlsAgent(): https.Agent {
  const certsDir = path.resolve(process.cwd(), 'certs', 'basspago3');
  try {
    return new https.Agent({
      cert: fs.readFileSync(path.join(certsDir, 'BASSPAGO_3.crt')),
      key:  fs.readFileSync(path.join(certsDir, 'BASSPAGO_3.key')),
...
export class QRCodes3Acquirer implements IAcquirer {
  readonly slug = 'qrcodes3';
  ...
  async createPix(
    ...
    if (!credentials.pixKey) {
      throw new Error('Chave PIX não configurada para BaassPago (QRCodes3)');
    }
    ...
    try {
      ...
      throw new Error(`BaassPago3 /cob falhou: ${detail}`);
    }
  ...
  async validateCredentials(credentials: AcquirerCredentials): Promise<boolean> {
    try {
      ...
      // Então registramos ${base}/qrcodes3 e eles chamam ${base}/qrcodes3/pix
      if (credentials.pixKey) {
        const base = (process.env.TELEGRAM_WEBHOOK_URL ?? 'http://localhost:3001/api/webhooks/telegram')
          .replace(/\/telegram$/, '');
        const webhookUrl = `${base}/qrcodes3/pix`;
