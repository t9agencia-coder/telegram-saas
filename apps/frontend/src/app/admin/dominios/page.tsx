...
import {
  Globe, Plus, Loader2, Pencil, Trash2, Check, Copy,
  ToggleLeft, ToggleRight, Star, X, AlertCircle,
...
export default function DominiosPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [domainsRes, infoRes] = await Promise.all([
        ...
        api.get<{ serverIp: string }>('/admin/domains/server-info'),
      ])
  ...
  const openCreate = () => { setInput(''); setEditing(null); setError(''); setModal('create') }
  const openEdit   = (d: Domain) => { setInput(d.domain); setEditing(d); setError(''); setModal('edit') }
  const closeModal = () => { setModal(null); setEditing(null); setInput(''); setError('') }
  ...
  const save = async () => {
    ...
    try {
      if (modal === 'create') {
        const created = await api.post<Domain>('/admin/domains', { domain: input.trim() })
        setDomains(prev => [created, ...prev])
      } else if (modal === 'edit' && editing) {
        const updated = await api.put<Domain>(`/admin/domains/${editing.id}`, { domain: input.trim() })
        setDomains(prev => prev.map(d => d.id === updated.id ? { ...d, ...updated, linksCount: d.linksCount } : d))
      }
      closeModal()
    } catch (e: any) {
  ...
  const toggle = async (d: Domain) => {
    if (!d.isActive && (d.dnsStatus !== 'active' || d.sslStatus !== 'active')) return
    try {
      const endpoint = d.isActive ? 'deactivate' : 'activate'
      const updated = await api.patch<Domain>(`/admin/domains/${d.id}/${endpoint}`)
      setDomains(prev => prev.map(x => x.id === d.id ? { ...x, isActive: updated.isActive } : x))
    } catch (e: any) {
      setError(e.message || 'Erro ao alterar status')
    }
  }

  const setDefault = async (d: Domain) => {
    if (d.isDefault || d.dnsStatus !== 'active' || d.sslStatus !== 'active') return
    try {
      await api.patch(`/admin/domains/${d.id}/default`)
      setDomains(prev => prev.map(x => ({
        ...x,
        isDefault: x.id === d.id,
        isActive:  x.id === d.id ? true : x.isActive,
      })))
    } catch (e: any) {
  ...
  const togglePicker = async (d: Domain) => {
    try {
      const updated = await api.patch<Domain>(`/admin/domains/${d.id}/toggle-picker`)
      setDomains(prev => prev.map(x => x.id === d.id ? { ...x, hiddenFromPicker: updated.hiddenFromPicker } : x))
    } catch (e: any) {
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-white">Domínios</h1>
          <p className="text-xs text-[#555] mt-0.5">Domínios customizados com SSL automático para geração de links</p>
        </div>
        <button
          onClick={openCreate}
          className="h-9 px-4 rounded-[4px] bg-primary hover:bg-[#FF1F2D] text-white text-sm font-medium transition-all flex items-center gap-2"
        >
      ...
      {error && (
        <div className="flex items-center gap-2 p-3 rounded-[4px] bg-red-500/10 border border-red-500/20 text-sm text-red-400 mb-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-sm text-[#555]">Nenhum domínio cadastrado</p>
                  ...
                  <tr className="border-b border-white/[0.04] last:border-0 hover:bg-white/[0.01] transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {d.hiddenFromPicker && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-white/[0.04] text-[#777] border border-white/[0.08]">
                            <EyeOff className="h-2.5 w-2.5" />
                            Oculto pra novos links
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1">
                        {/* Padrão — só para totalmente ativos */}
                        {!d.isDefault && d.dnsStatus === 'active' && d.sslStatus === 'active' && (
                          <button onClick={() => setDefault(d)} title="Definir como padrão"
                            className="h-7 w-7 flex items-center justify-center rounded-[3px] text-[#444] hover:text-amber-400 hover:bg-amber-400/5 transition-colors">
                            <Star className="h-3.5 w-3.5" />
                          </button>
                        )}
                        {/* Toggle — só para verificados e não-padrão */}
                        {d.dnsStatus === 'active' && d.sslStatus === 'active' && !d.isDefault && (
                          <button onClick={() => toggle(d)} title={d.isActive ? 'Desativar' : 'Ativar'}
                            className="h-7 w-7 flex items-center justify-center rounded-[3px] text-[#444] hover:text-white hover:bg-white/5 transition-colors">
                            {d.isActive
                              ? <ToggleRight className="h-3.5 w-3.5 text-green-400" />
                              : <ToggleLeft  className="h-3.5 w-3.5" />}
                          </button>
                        )}
                        {/* Ocultar de novos redirecionadores — disponível pra qualquer domínio, inclusive o padrão */}
                        <button onClick={() => togglePicker(d)}
                          title={d.hiddenFromPicker ? 'Disponível pra novos redirecionadores' : 'Ocultar de novos redirecionadores'}
                          className="h-7 w-7 flex items-center justify-center rounded-[3px] text-[#444] hover:text-white hover:bg-white/5 transition-colors">
                          {d.hiddenFromPicker
          ...
          <div className="bg-[#141414] border border-white/[0.08] rounded-[6px] w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-sm font-semibold text-white">
                {modal === 'create' ? 'Novo Domínio' : 'Editar Domínio'}
              </h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-[#444] mt-1.5">
                  Após cadastrar: configure o DNS e o SSL será emitido automaticamente via Let's Encrypt.
                </p>
              </div>
              {modal === 'edit' && (
                <div className="flex items-start gap-2 p-2.5 rounded-[3px] bg-amber-500/[0.06] border border-amber-500/15">
                  <AlertTriangle className="h-3.5 w-3.5 text-amber-400 shrink-0 mt-0.5" />
                  <p className="text-[11px] text-amber-400">
                    Alterar o domínio requer re-verificação do DNS e novo certificado SSL.
                  </p>
                </div>
              )}
